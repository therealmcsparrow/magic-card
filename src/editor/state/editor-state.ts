...
export class EditorStateManager {
  ...
  addColumn(rowIndex: number): void {
    const config = deepClone(this.state.config);
    config.rows[rowIndex].columns.push({
      id: generateId('col'),
      modules: [],
    });
    this.updateConfig(config);
  ...
  deleteColumn(rowIndex: number, colIndex: number): void {
    const config = deepClone(this.state.config);
    config.rows[rowIndex].columns.splice(colIndex, 1);
    if (config.rows[rowIndex].columns.length === 0) {
      config.rows.splice(rowIndex, 1);
    }
    this.state.selectedPath = null;
    this.updateConfig(config);
  ...
  moveModule(
    ...
    this.updateConfig(config);
    this.setSelectedPath({ rowIndex: toRow, columnIndex: toCol, moduleIndex: toIdx });
  }
