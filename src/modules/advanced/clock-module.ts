  ...
  renderGeneralTab(
    ...
    return html`
      <div class="mc-tab-content">
        ${renderToggleField('12-Hour Format', c.format_12h, (v) => onChange({ ...c, format_12h: v }), 'format_12h', syncCtx)}
        ${renderToggleField('Show Seconds', c.show_seconds, (v) => onChange({ ...c, show_seconds: v }), 'show_seconds', syncCtx)}
        ${renderToggleField('Show Date', c.show_date, (v) => onChange({ ...c, show_date: v }), 'show_date', syncCtx)}
        ${renderTextField('Date Format', c.date_format, (v) => onChange({ ...c, date_format: v }), 'date_format', syncCtx)}
      </div>
    `;
  }
}
